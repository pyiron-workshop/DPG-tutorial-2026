...
def _prepare_potential_and_structure(potential, structure):
    ...
    potential = get_potential_by_name(potential_name=potential, structure=structure)

    pair_style = []
    pair_coeff = []
    
    pair_style.append(" ".join(potential["Config"].to_list()[0][0].strip().split()[1:]))
    pair_coeff.append(" ".join(potential["Config"].to_list()[0][1].strip().split()[1:]))

    #now prepare the list of elements
    elements = list(potential["Species"])[0]
    elements_from_pot = list(potential["Species"])[0]

    lmp_structure = LammpsStructure()
    lmp_structure.potential = potential
    lmp_structure.atom_type = "atomic"
    lmp_structure.el_eam_lst = list(potential["Species"])[0]
